...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    // $user_data=$users->GetUser();
        ...
        FROM `order`
        INNER JOIN `users` ON order.user_id = users.user_id ORDER BY order.order_id DESC";
     ...
     <tr>
          ...
          <td><span><?php  echo ucfirst($oder_status);?></span>
            <select name="changeorderstatus" id="changeorderstatus_<?php echo $order_id;?>" onchange="changeOrderStatus(<?php echo $order_id;?>)">
                <option value="">Select</option>
                <option value="completed" <?php if($oder_status=='completed'){?> selected <?php }?>>Completed</option>
                <option value="pending" <?php if($oder_status=='pending'){?> selected <?php }?>>Pending</option>
            </select>
          </td>
          <td><span><?php echo ucfirst($payment_status);?></span>
            <select name="changepaymentstatus" id="changepaymentstatus_<?php echo $order_id;?>" onchange="changePaymentStatus(<?php echo $order_id;?>)">
                <option value="">Select</option>
                <option value="completed" <?php if($payment_status=='completed'){?> selected <?php }?>>Completed</option>
                <option value="pending" <?php if($payment_status=='pending'){?> selected <?php }?>>Pending</option>
                option
            </select>
          </td>
...
<script type="text/javascript">
  function changeOrderStatus(orderId) {
    var svalue = document.getElementById("changeorderstatus_"+orderId).value;
    if (svalue!='') {
      $.post("../Controller/updateOrderController.php", {orderId:orderId,svalue:svalue,action:'update_order_status'},function(result){
        //$("#replace_"+orderId).html(result);
          location.reload();
      });
    }else{
  ...
  function changePaymentStatus(orderId) {
    var svalue = document.getElementById("changepaymentstatus_"+orderId).value;
    if (svalue!='') {
      $.post("../Controller/updateOrderController.php", {orderId:orderId,svalue:svalue,action:'update_payment_status'},function(result){
        //$("#replacestatus_"+orderId).html(result);
        location.reload();
      });
    }else{
      alert('please select status')
